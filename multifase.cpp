...
double LatticeBoltzmann::gradient_y(int ix, int iy){
    ...
    int j1 = (iy + 1) % Ly; // siguiente columna, con envoltura
    int j2 = (iy + 2) % Ly;
    int j3 = (iy + 3) % Ly;
    
    int jm1 = (iy - 1 + Ly) % Ly; // columna anterior, con envoltura
    int jm2 = (iy - 2 + Ly) % Ly;
    int jm3 = (iy - 3 + Ly) % Ly;
...
int main(void){
    InicieAnimacion();
    LatticeBoltzmann inestability;
    int t, tmax = 400000;
    double Ux0 = 0; double Uy0 = 0;
    inestability.Start(Ux0, Uy0);
    for(t=0;t<tmax;t++){
        //std::cout<<std::endl<<"colision "<<t+1<<std::endl;
       inestability.Collision();
       inestability.ImposeFields();
       inestability.Advection();
       if (t%500 == 0){
    
       inestability.print("datos.dat");
       InicieCuadro();
       cout<<", 'datos.dat' w image";
       TermineCuadro();}
    }
    //inestability.print("datos.dat");
    return 0;
